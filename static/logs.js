...
(() => {
  ...
  const oldOpenLogs=window.openLogs; window.openLogs=async function(id){ await oldOpenLogs(id); st.log.query=''; st.log.level=''; st.log.time='0'; st.log.regex=false; AppStore.persist(); syncInputs(); updateLogSource(); applyLogFilters(); };
  const oldOpenInstaller=window.openInstallerLogs; window.openInstallerLogs=async function(){ await oldOpenInstaller(); st.log.query=''; st.log.level=''; st.log.time='0'; st.log.regex=false; AppStore.persist(); syncInputs(); updateLogSource(); applyLogFilters(); };
