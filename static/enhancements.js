...
(() => {
  ...
  // ---- diagnostics page ----
  function ensureDiagnostics(){if($id('diagBack'))return;const main=document.querySelector('main'),panel=document.createElement('section');panel.id='diagBack';panel.className='workspace-panel';panel.dataset.workspacePanel='true';panel.innerHTML='<div class="workspace-view"><header><button aria-label="Back" class="x" onclick="closeModal(\'diagBack\')" title="Back"><span class="material-symbols-outlined" aria-hidden="true">arrow_back</span></button><div><h2>Diagnostics</h2><span class="sub">system · Docker · registry · remotes · recent errors</span></div><div style="margin-left:auto;display:flex;gap:8px;align-items:center"><button class="btn btn-line btn-sm" onclick="copyDiagnostics()"><span class="material-symbols-outlined" aria-hidden="true">content_copy</span><span>Copy</span></button><button class="btn btn-primary btn-sm" onclick="downloadDiagnostics()"><span class="material-symbols-outlined" aria-hidden="true">download</span><span>Download</span></button></div></header><div class="body"><div class="section-head"><div><h3>Support diagnostics</h3><p>System, Docker, registry, remote, storage, and recent backend information.</p></div></div><div id="diagContent" class="skeleton-list"><div class="skeleton-row"></div></div></div></div>';main.appendChild(panel);$id('maintBack')?.querySelector('[data-settings-tab="runtime"]')?.insertAdjacentHTML('beforeend','<div class="field"><label>Support diagnostics</label><button class="btn btn-line" onclick="openDiagnostics()">Open diagnostics</button></div>');}
  let diagData=null;
