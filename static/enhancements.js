...
(() => {
  ...
  window.openUpdatePolicy=async id=>{const it=INSTALLED.find(x=>x.id===id);const cur=it?.update_policy||{mode:it?.auto_update?'stable':'off',delay_days:0,maintenance_window:''};const r=await uiPrompt({title:'Update policy',message:'Modes: off, notify, stable, prerelease, scheduled',label:'Policy mode',value:cur.mode});if(!r.confirmed)return;const mode=r.value.trim().toLowerCase();await api(`/api/instances/${id}/update-policy`,{method:'PUT',body:JSON.stringify({mode,delay_days:cur.delay_days||0,maintenance_window:cur.maintenance_window||''})});operationNotice('Update policy changed',`${it?.label||id}: ${mode}`,'success');loadInstalled();};
  const oldDetails=window.detailsHtml;window.detailsHtml=function(it){let h=oldDetails(it);h=h.replace(/<label class="auto-control"[\s\S]*?<\/label>/,`<button class="btn btn-line" onclick="openUpdatePolicy('${it.id}')">Update policy</button>`);return h;};
