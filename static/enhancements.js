...
(() => {
  ...
  window.renderOperations=function(){
    ...
    const section=(name,title,rows,collapsed=false)=>`<section class="operation-group ${collapsed?'collapsed':''}"><button class="operation-group-head" onclick="this.parentElement.classList.toggle('collapsed')"><span>${title}</span><b>${rows.length}</b><i>⌄</i></button><div>${rows.map(item).join('')||'<p class="operation-empty">None</p>'}</div></section>`;
  ...
  window.bulkRegister=async function(){
    ...
    const choice=await uiPrompt({title:'Register selected integrations',message:'Enter the remote name or ID.',label:'Remote',value:(remotes.find(r=>r.id===REMOTES.active)||remotes[0]).name||remotes[0].id});if(!choice.confirmed)return;
  ...
  window.registerIntegration=async function(id,remoteId){
    remoteId=remoteId||REMOTES.active; if(!remoteId)return rawRegister(id,remoteId);
