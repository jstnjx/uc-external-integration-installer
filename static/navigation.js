...
(() => {
  ...
  function renderBreadcrumbs(){ const route=(location.hash||'#/installed').replace(/^#\/?/,''); const parts=labelFor(route); crumbs.innerHTML=parts.map((p,i)=>'<span'+(i===parts.length-1?' aria-current="page"':'')+'>'+esc(p)+'</span>').join('<b>'+msIcon('chevron_right')+'</b>'); crumbs.style.display=parts.length>1?'flex':'none'; AppStore.set({route}); }
